...
**4. 自适应界面（新手/专业模式切换）**
- **新手模式（默认）**：简化界面，隐藏专业术语，引导优先
- **专业模式（检测到或手动切换）**：批量操作、快捷键、高级设置
- **模式切换按钮**："切换到专业视图" / "切换到简化视图"
...
- **设计要求**：
  - **默认视图（新手模式）**：简洁的风格标签 + 质量指标
    - 示例："赛博朋克风格" | "电影感光效" | "霓虹配色"
    - 质量指标："✓ 256 位用户使用，成功率 94%"
  - **可选专业视图（专业模式）**：四维度卡片式呈现 + 专业术语
    - 光影：伦勃朗光 + 霓虹边缘光
    - 构图：黄金分割 + 低角度仰拍
    - 色彩：青橙色互补色配色 + 高饱和度
    - 艺术风格：赛博朋克 + 电影感
...
- **UX 选择**：默认新手模式，提供"切换到专业视图"按钮

**[效率感] → [批量操作 + 快捷键]**
- 批量上传（最多5张同风格图片）
- 快捷键支持（Ctrl+C 复制、Ctrl+S 保存、Ctrl+D 上传）
- 一键复制模版、一键保存到模版库
- **UX 选择**：为专业用户提供快捷键，但不强制新手使用
...
**3. 自适应情感体验**
- 新手用户：自信感、引导感（简化界面、术语解释）
- 专业用户：效率感、掌控感（批量操作、快捷键）
- 模式切换让用户选择情感体验复杂度
...
- **image_analyzer 避免**：自适应界面（新手模式默认，专业模式可选），高级参数可折叠
...
**4. 一刀切的界面**
- **反模式特征**：新手和专业用户看到相同界面，无法调整复杂度
- **用户感受**：新手困惑（"我不懂这个术语"）、专业用户沮丧（"效率太低"）
- **竞品案例**：大多数 AI 工具没有用户类型区分
- **image_analyzer 避免**：自适应界面（新手/专业模式切换）
...
--accent-soft: #BFDBFE;      /* 柔和蓝 */
--border-subtle: #E2E8F0;    /* 轻边框 */
...
### User Tiered Experience

基于用户焦点小组的反馈，image_analyzer 提供三层用户体验：

| 用户类型 | 核心需求 | 体验层次 | 关键功能 | 界面复杂度 |
|---------|---------|---------|---------|-----------|
| **Mike**<br>（灵感捕捉者）| 简单易懂 | 简单层 | • 风格标签<br>• 质量指标<br>• 复制按钮 | 低（新手模式）|
| **Sarah**<br>（AI 创作者）| 快速可用 | 实用层 | • 完整模版<br>• 参数预览<br>• 一键复制 | 中（自适应）|
| **Emma**<br>（专业设计师）| 专业透明 | 专业层 | • 详细分析<br>• 用户评论<br>• 参数透明 | 高（专业模式）|

**Mike（简单层）：**
- 默认视图：风格标签（"赛博朋克风格"）+ 质量指标 + 复制按钮
- 隐藏：四维度专业分析（点击"查看详情"展开）
- 一键复制：突出显示，无需思考

**Sarah（实用层）：**
- 默认视图：风格标签 + 质量指标 + 完整模版预览 + 复制按钮
- 可选视图：四维度专业分析（折叠）
- 参数完整性：突出显示所有参数

**Emma（专业层）：**
- 默认视图：四维度专业分析 + 质量指标 + 完整模版 + 用户评论
- 参数透明：显示参数来源和计算逻辑
- 高级选项：批量操作、JSON 导出
...
| **P2（可选）** | 专业分析 | ⚠️ 可选 | 点击展开，不默认显示<br>包含：四维度分析、专业术语解释 |
...
3. **查看结果**：
   - **默认视图**（简单层）：风格标签 + 质量指标 + 完整模版预览 + 复制按钮
   - **可选视图**（专业层）：四维度专业术语（点击展开）
...
5. **专业术语渐进披露** - 四维度分析支持点击展开，新手隐藏、专业用户可见
...
| 用户类型 | 默认视图 | 可选视图 | 界面复杂度 |
|---------|---------|---------|-----------|
| **Mike（灵感捕捉者）** | 风格标签 + 质量指标 + 复制按钮 | 四维度分析（点击展开） | 低 |
| **Sarah（AI 创作者）** | 完整模版 + 实时预览 + 复制按钮 | 四维度分析（点击展开） | 中 |
| **Emma（专业设计师）** | 四维度分析 + 模版 + 生成结果 | - | 高 |
...
- 模式切换使用 CSS 变量 + `[data-theme]` 属性，零 JS 开销
...
#### ModeSwitch（新手/专业模式切换器）

**Purpose:** 实现自适应界面切换，是用户分层体验的核心交互组件
...
**Behavior:**
- 切换时通过 `[data-theme]` 属性控制 CSS 变量
- 500ms 平滑过渡动画（Phase 2 实现）
...
<ToggleButtonGroup
  ...
  aria-label="选择界面模式"
>
  <ToggleButton value="beginner" aria-label="切换到新手模式">
    新手模式
  </ToggleButton>
  <ToggleButton value="professional" aria-label="切换到专业模式">
    专业模式
  </ToggleButton>
</ToggleButtonGroup>
```
...
| **Custom** | MUI Paper + Tailwind | ModeSwitch, TemplateEditor | 玻璃态 + CSS 变量 |
...
| 改进项 | 来源 | 影响 |
|--------|------|------|
| 模式切换用 CSS 变量 | Alex（性能） | 性能提升 40%，零 JS 开销 |
