...
**Practice Mode (UX 需求):**
- FR-PM1: 新用户首次使用可获得练习 credit（不消耗正式额度）
- FR-PM2: 练习模式分析结果标注"练习"标识
- FR-PM3: 练习模式可体验完整功能但限制保存/导出
...
| Epic | 标题 | FRs | 用户价值 |
|------|------|-----|---------|
| **Epic 0** | 项目初始化与基础设施 | - | 开发团队可以开始功能开发 |
| **Epic 1** | 用户认证与账户系统 | FR1-5 | 用户可以注册登录、管理账户 |
| **Epic 2** | 图片上传与管理 | FR6-12, FR67 | 用户可以上传和管理图片（支持移动端） |
| **Epic 3** | AI 风格分析 | FR13-18, FR74, FR76, FR78, FR79 | 用户可以获取专业风格分析 |
| **Epic 4** | 内容安全与合规 | FR50-56 | 用户安全地上传和分析图片 |
...
| **Epic 8** | 订阅与支付系统 | FR42-49, FR68, FR80, FR81 | 用户可以购买订阅和管理积分 |
...
**用户成果：** 开发团队拥有完整的项目骨架和开发环境，可以开始功能开发。
...
**数据库 Schema（在 Epic 1 创建）：**
- `users` 表：用户账户信息、OAuth 信息、积分余额
...
#### Epic 3: AI 风格分析
**FRs:** FR13-18, FR74, FR76, FR78, FR79
...
- 置信度标注与低置信度警告
- 用户质量反馈机制
- **后台队列处理（FR78）：** 分析超时后将任务加入后台队列
- **队列透明化（FR79）：** 显示当前等待队列中的任务数量

**数据库 Schema（在 Epic 3 创建）：**
- `analysis_results` 表：风格分析结果存储
...
**数据库 Schema（在 Epic 4 创建）：**
- `moderation_logs` 表：内容审核日志
...
**数据库 Schema（在 Epic 5 创建）：**
- `templates` 表：用户生成的提示词模版
...
**数据库 Schema（在 Epic 7 扩展）：**
- `templates` 表扩展：添加收藏、标签、分类字段
- `template_usage` 表：模版使用统计

---

#### Epic 8: 订阅与支付系统
**FRs:** FR42-49, FR68, FR80, FR81
...
- Credit 扣除与消费记录
- Creem 支付集成
- **管理员模型配置（FR68）：** 管理员可以配置启用/禁用不同的模型提供商
- **Practice Mode 支持：** 新用户首次使用可获得练习 credit（不消耗正式额度）

**数据库 Schema（在 Epic 8 创建）：**
- `credits` 表：用户积分交易记录
- `subscriptions` 表：用户订阅信息
- `credit_transactions` 表：积分充值和消费明细
