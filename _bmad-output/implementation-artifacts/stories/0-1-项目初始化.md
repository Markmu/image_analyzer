...
- 评审修复：lint-staged 增加 ESLint；新增 `.vscode/settings.json` 支持保存自动格式化；复验 `npm run lint`、`npm run build`、`npm run dev`（启动后停止）
...
- 2026-02-03: 修复评审问题（lint-staged 运行 ESLint、补充保存自动格式化配置、复验 lint/build/dev）
