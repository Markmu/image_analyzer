...
- [ ] **Task 1: 创建生成功能数据结构和类型定义** (AC: 1, 2, 6, 7)
  - [ ] 1.1 定义 `ImageGenerationOptions` 接口(模型提供商、分辨率、数量)
  - [ ] 1.2 定义 `ImageGenerationResult` 接口(图片 URL、元数据、credit 消耗)
  - [ ] 1.3 定义 `GenerationProvider` 接口(提供商信息、可用模型列表)
  - [ ] 1.4 定义 `ResolutionConfig` 配置(按订阅等级的分辨率选项)
  - [ ] 1.5 创建生成选项的常量配置(DEFAULT_RESOLUTIONS, GENERATION_LIMITS)

- [ ] **Task 2: 实现生图 API 客户端** (AC: 2, 3, 6)
  - [ ] 2.1 创建 `src/lib/ai/image-generation.ts` API 函数
  - [ ] 2.2 实现生图模型提供商列表获取(从 Replicate API)
  - [ ] 2.3 实现基于模版的提示词构建逻辑
  - [ ] 2.4 集成 Replicate API 调用(使用生图模型)
  - [ ] 2.5 实现 credit 消费逻辑(根据分辨率和数量)
  - [ ] 2.6 实现生成进度轮询逻辑

- [ ] **Task 3: 实现 GenerateButton 组件** (AC: 1, 3)
  - [ ] 3.1 创建 `GenerateButton` 主组件
  - [ ] 3.2 实现按钮状态逻辑(启用/禁用/加载中)
  - [ ] 3.3 集成 Lucide ImageIcon 或 Wand2 图标
  - [ ] 3.4 实现加载动画(旋转动画)
  - [ ] 3.5 实现生成选项选择对话框(模型、分辨率、数量)

- [ ] **Task 4: 实现生成结果预览对话框** (AC: 4, 8)
  - [ ] 4.1 创建 `GenerationPreviewDialog` 组件
  - [ ] 4.2 实现图片预览视图(支持缩放)
  - [ ] 4.3 实现图片元数据显示(模型、时间、分辨率)
  - [ ] 4.4 实现"重新生成"、"保存"、"分享"按钮
  - [ ] 4.5 应用 Glassmorphism 样式

- [ ] **Task 5: 集成内容安全检查** (AC: 5)
  - [ ] 5.1 调用 Story 4.1 的内容安全检查逻辑(模版验证)
  - [ ] 5.2 调用视觉模型进行生成图片内容审核
  - [ ] 5.3 实现不安全内容的警告和阻止逻辑
  - [ ] 5.4 实现不安全图片的删除和 credit 退款

- [ ] **Task 6: 实现分辨率和订阅等级配置** (AC: 6)
  - [ ] 6.1 创建 `ResolutionSelector` 组件
  - [ ] 6.2 实现按订阅等级的分辨率选项逻辑
  - [ ] 6.3 实现用户订阅状态检查
  - [ ] 6.4 显示升级提示(当用户选择更高分辨率选项时)

- [ ] **Task 7: 实现批量生成功能** (AC: 7)
  - [ ] 7.1 实现批量生成逻辑(并发 API 调用)
  - [ ] 7.2 实现批量进度显示(已完成/总数)
  - [ ] 7.3 实现批量图片预览(网格布局)
  - [ ] 7.4 实现批量下载功能(打包为 ZIP)

- [ ] **Task 8: 实现图片保存和分享功能** (AC: 4)
  - [ ] 8.1 实现图片保存到本地功能(下载)
  - [ ] 8.2 集成 X/Twitter 分享 API
  - [ ] 8.3 实现分享链接生成(包含分享追踪)
  - [ ] 8.4 实现分享奖励机制(credit 奖励,占位符实现)

- [ ] **Task 9: 集成到模版编辑器** (AC: 1)
  - [ ] 9.1 在 `TemplateEditor` 组件中集成 `GenerateButton`
  - [ ] 9.2 实现生成结果与模版的关联
  - [ ] 9.3 确保与现有按钮的布局协调
  - [ ] 9.4 实现生成历史记录(可选,链接到 Epic 7)

- [ ] **Task 10: 实现 Toast 通知和用户反馈** (AC: 3, 5)
  - [ ] 10.1 实现生成开始通知(Toast)
  - [ ] 10.2 实现生成成功通知(Toast)
  - [ ] 10.3 实现生成失败通知(Toast + 错误信息)
  - [ ] 10.4 实现内容安全警告通知
...
- [ ] **Task 11: 单元测试**
  ...
  - [ ] 11.2 测试分辨率配置逻辑
  - [ ] 11.3 测试提示词构建逻辑
  - [ ] 11.4 测试 credit 消费计算逻辑

- [ ] **Task 12: 集成测试**
  - [ ] 12.1 测试完整生成流程(点击按钮 → 选择选项 → 预览结果 → 保存)
  - [ ] 12.2 测试生成失败场景(API 错误、网络错误)
  - [ ] 12.3 测试内容安全检查场景
  - [ ] 12.4 测试不同订阅等级的分辨率选项
