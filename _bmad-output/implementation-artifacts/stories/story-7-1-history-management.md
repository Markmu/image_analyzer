...
- [x] **Task 1: 创建数据库 Schema 和迁移** (AC: 1, 6)
  - [x] 1.1 定义 `analysis_history` 表结构
  - [x] 1.2 创建 Drizzle 迁移脚本
  - [x] 1.3 运行迁移并验证表结构
  - [x] 1.4 添加自动清理机制（数据库触发器或应用层逻辑）

- [x] **Task 2: 创建历史记录 API 路由** (AC: 2, 3, 7)
  - [x] 2.1 创建 `GET /api/history` 端点（获取历史记录列表）
  - [x] 2.2 创建 `GET /api/history/[id]` 端点（获取单条历史记录详情）
  - [x] 2.3 创建 `POST /api/history/[id]/reuse` 端点（基于历史模版创建新分析）
  - [x] 2.4 实现身份验证和授权检查
  - [x] 2.5 添加分页和排序参数

- [x] **Task 3: 实现历史记录服务层** (AC: 1, 6)
  - [x] 3.1 创建 `saveToHistory` 函数（保存分析记录）
  - [x] 3.2 创建 `getHistoryList` 函数（获取用户历史列表）
  - [x] 3.3 创建 `getHistoryDetail` 函数（获取单条记录详情）
  - [x] 3.4 创建 `reuseFromHistory` 函数（基于历史模版创建新分析）
  - [x] 3.5 实现 FIFO 自动清理逻辑（保留最近 10 条）

- [x] **Task 4: 创建历史记录 UI 组件** (AC: 2, 5)
  - [x] 4.1 创建 `HistoryList` 组件（历史记录列表）
  - [x] 4.2 创建 `HistoryCard` 组件（单条历史记录卡片）
  - [x] 4.3 创建 `HistoryDetail` 组件（历史记录详情页面）
  - [x] 4.4 应用 Glassmorphism 样式（`ia-glass-card`）
  - [x] 4.5 集成 Lucide 图标（History, Clock, Eye, RefreshCw）

- [x] **Task 5: 实现历史记录页面** (AC: 2, 3, 5)
  - [x] 5.1 创建 `/history` 页面（历史记录列表页）
  - [x] 5.2 创建 `/history/[id]` 页面（历史记录详情页）
  - [x] 5.3 实现响应式布局（移动端/平板端/桌面端）
  - [x] 5.4 添加加载状态和错误处理

- [x] **Task 6: 集成历史记录保存逻辑** (AC: 1, 6)
  - [x] 6.1 在分析完成后自动调用 `saveToHistory`
  - [x] 6.2 修改 `AnalysisCard` 组件，添加保存到历史逻辑
  - [x] 6.3 确保只保存成功的分析记录

- [x] **Task 7: 实现"重新使用模版"功能** (AC: 4)
  - [x] 7.1 在历史记录详情页添加"重新使用"按钮
  - [x] 7.2 实现模版加载到分析界面的逻辑
  - [x] 7.3 添加模版可编辑功能
  - [x] 7.4 确保不消耗额外 credit（只在上传新图片时扣除）

- [x] **Task 8: 单元测试和集成测试** (AC: 1-7)
  - [x] 8.1 测试历史记录保存逻辑（FIFO 清理）
  - [x] 8.2 测试 API 路由（列表、详情、重新使用）
  - [x] 8.3 测试授权控制（用户只能访问自己的数据）
  - [x] 8.4 测试 UI 组件渲染和交互
  - [x] 8.5 测试响应式布局

- [x] **Task 9: E2E 测试**
  - [x] 9.1 测试完整流程：上传 → 分析 → 自动保存到历史
  - [x] 9.2 测试历史记录列表浏览
  - [x] 9.3 测试历史记录详情查看
  - [x] 9.4 测试基于历史模版创建新分析
  - [x] 9.5 测试 FIFO 自动清理（第 11 条记录删除第 1 条）
  - [x] 9.6 测试移动端交互
